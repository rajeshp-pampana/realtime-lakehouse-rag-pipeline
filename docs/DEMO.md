...
What differs between paths is speed, setup cost, and **how much of it has
actually been verified**.

Legend: **✅ CI** verified by CI on every push · **✅ local** verified by hand on
this machine · **⚙️ built, unverified** implemented but never run · **—** not
deployed on that path.

| Feature | Native (Windows) | Docker Compose | Kubernetes (kind) |
|---|---|---|---|
| Curated prices, ticks, lakehouse stats | ✅ local | ✅ CI | ✅ CI |
...
| Kafka → Spark → Delta streaming | manual | ✅ CI `--profile streaming` | — |
...
**Do not claim in-cluster briefings work.** The Helm chart can deploy Ollama and
CI proves the chart still renders and deploys — but CI runs it with
`ollama.enabled=false`, and a briefing has never been generated in-cluster. The
k8s smoke test proves the chart is valid, not that in-cluster RAG works.

**No briefing is verified by CI on any path.** A ~12-minute CPU generation would
dominate a 4-minute pipeline, so the compose briefing was generated by hand.
...
4. **Generate Briefing.** When it returns, point at the *"Grounded in: ..."*
   ...
   If asked how you know retrieval works: there is a committed 36-question
   labelled evaluation (`docs/eval/retrieval_eval.yaml`), precision@1 is 0.917,
   and chunking the corpus is what took it there from 0.528.
